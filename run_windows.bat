...
if "%ERRORLEVEL%" NEQ "0" (
    ...
    echo Then make sure it is accessible in your PATH.
    echo ------------------------------------------
    pause
    exit /b 1
)
...
if "%ERRORLEVEL%" NEQ "0" (
    echo Error: curl not found. Required for downloads. Please install it.
    goto :error_exit
)
...
if not exist "%ACTIVATE_SCRIPT%" (
    ...
    if "%ERRORLEVEL%" NEQ "0" (
        echo Error: Virtual env creation failed. Make sure Python %PYTHON_VERSION% is installed.
        goto :error_exit
    )
) else (
...
if "%SCRIPT_EXIT_CODE%" NEQ "0" (
    echo Python Script Failed (Code: %SCRIPT_EXIT_CODE%).
